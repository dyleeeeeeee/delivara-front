...
(() => {
  ...
  class liquidGLRenderer {
    ...
    /* -----------------------------
     *  Detach a lens (React unmount). Splices it out and tears down the DOM
     *  bits it created so navigations don't leak shadow/mirror nodes.
     * ----------------------------- */
    removeLens(lens) {
      if (!lens) return;
      const i = this.lenses.indexOf(lens);
      if (i !== -1) this.lenses.splice(i, 1);
      this._pendingReveal = (this._pendingReveal || []).filter((l) => l !== lens);
      try {
        if (lens._unbindTiltHandlers) lens._unbindTiltHandlers();
        if (lens._destroyMirrorCanvas) lens._destroyMirrorCanvas();
        if (lens._shadowEl) {
    ...
    _updateDynamicVideos() {
      ...
      this._videoNodes.forEach((vid) => {
        ...
        // Canvases (e.g. a live Mapbox map registered via registerLiveCanvas)
        // have no readyState — only gate real <video> elements on it.
        if (this._isIgnored(vid) || (vid.tagName === "VIDEO" && vid.readyState < 2)) return;
    ...
    /* -----------------------------
     *  Register a live <canvas> (e.g. a Mapbox map) as a refraction source.
     *  Routed through the per-frame video compositing path: each frame the
     *  canvas is drawImage'd over the static snapshot and uploaded to the GL
     *  texture, so the glass refracts the moving map at frame rate. The map
     *  must be created with preserveDrawingBuffer:true so its pixels survive
     *  the readback.
     * ----------------------------- */
    addLiveCanvas(el) {
      if (!el || el.tagName !== "CANVAS") return;
      if (this._isIgnored(el)) return;
      if (!this._videoNodes) this._videoNodes = [];
      if (this._videoNodes.indexOf(el) !== -1) return;
      this._videoNodes.push(el);
    }

    removeLiveCanvas(el) {
      if (!this._videoNodes) return;
      const i = this._videoNodes.indexOf(el);
      if (i !== -1) this._videoNodes.splice(i, 1);
    }
  ...
  /* --------------------------------------------------
   *  Public helper: register a live <canvas> (e.g. Mapbox) so the glass
   *  refracts it at frame rate. Returns an unregister function.
   * ------------------------------------------------*/
  window.liquidGL.registerLiveCanvas = function (el) {
    const renderer = window.__liquidGLRenderer__;
    if (!renderer || !renderer.addLiveCanvas || !el) return () => {};
    renderer.addLiveCanvas(el);
    return () => renderer.removeLiveCanvas && renderer.removeLiveCanvas(el);
  };
